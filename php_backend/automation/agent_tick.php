...
$sitemapInfo = null;
$indexing = null;
if (!$dryRun && count(array_filter($publishedNow, static fn(array $item): bool => ($item['status'] ?? '') === 'published')) > 0) {
    $sitemapInfo = write_dynamic_sitemap($conn);
    $indexing = ping_search_engines($sitemapInfo['sitemapUrl']);
}

automation_log_event($conn, 'agent_tick', $dryRun ? 'Agent dry run executed.' : 'Agent tick executed.', null, [
    'dryRun' => $dryRun,
