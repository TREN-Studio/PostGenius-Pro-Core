...
function agent_select_asin_groups(PDO $conn, array $keywords, int $desiredJobs): array
{
    ...
    foreach (agent_trending_feed_definitions() as $feed) {
        ...
        while (count($available) >= 3 && count($groups) < $desiredJobs) {
            $picked = agent_select_same_family_asins(array_slice($available, 0, 12), $config);
            if (count($picked) < 3) {
                $available = array_slice($available, 1);
                continue;
            }

            $available = array_values(array_filter(
                $available,
                static fn(string $asin): bool => !in_array($asin, $picked, true)
            ));
...
function agent_select_same_family_asins(array $candidateAsins, array $config): array
{
    $candidateAsins = array_values(array_unique(array_map('strtoupper', array_filter($candidateAsins))));
    if (count($candidateAsins) < 3) {
        return [];
    }

    $orderMap = array_flip($candidateAsins);
    $products = [];
    foreach ($candidateAsins as $asin) {
    ...
    $family = review_generator_filter_products_by_family($products, 'Trending Now');
    $filtered = array_values($family['filtered'] ?? []);
    if (count($filtered) < 3) {
        return [];
    }

    usort($filtered, static function (array $left, array $right) use ($orderMap): int {
        $leftAsin = strtoupper(trim((string)($left['asin'] ?? '')));
        $rightAsin = strtoupper(trim((string)($right['asin'] ?? '')));
        return ((int)($orderMap[$leftAsin] ?? 999)) <=> ((int)($orderMap[$rightAsin] ?? 999));
    });

    $picked = [];
    foreach ($filtered as $product) {
        $asin = strtoupper(trim((string)($product['asin'] ?? '')));
        if ($asin !== '' && !in_array($asin, $picked, true)) {
            $picked[] = $asin;
        }
        if (count($picked) >= 3) {
            break;
